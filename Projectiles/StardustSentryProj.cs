...
namespace StormDiversSuggestions.Projectiles
{
    ...
    public class StardustSentryProj : ModProjectile
    {
        ...
        public override void AI()
        {
           ...
           // projectile.TurretShouldPersist();
            ...
            for (int i = 0; i < 1; i++)
            {
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 111, projectile.velocity.X, projectile.velocity.Y, 120, default, .7f);   //this make so when this projectile is active has dust around , change PinkPlame to what dust you want from Terraria, or add mod.DustType("CustomDustName") for your custom dust
        ...
        public override void Kill(int timeLeft)
        {
            ...
            {
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 111, projectile.velocity.X * 1.2f, projectile.velocity.Y * 1.2f, 120, default, 0.5f);   //this make so when this projectile disappear will spawn dust, change PinkPlame to what dust you want from Terraria, or add mod.DustType("CustomDustName") for your custom dust
                Main.dust[dust].noGravity = true; 
    ...
    public class StardustSentryProj2 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (dusttime >= 5)
            {
                int dust = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 111, projectile.velocity.X, projectile.velocity.Y, 120, default, 0.8f);   //this make so when this projectile is active has dust around , change PinkPlame to what dust you want from Terraria, or add mod.DustType("CustomDustName") for your custom dust
                Main.dust[dust].noGravity = true;
        ...
        public override void Kill(int timeLeft)
        {
            if (projectile.owner == Main.myPlayer)
            {
                ...
                for (int i = 0; i < 10; i++)
                {
                    ...
                    var dust = Dust.NewDustDirect(projectile.Center, 0, 0, 111);
